...
class TodoController extends Todo
{
  ...
  public function displayTodo($todo_content, $todo_author, $todo_user_id)
  {
    $this->createTodo($todo_content, $todo_author, $todo_user_id);
  }

  public function displayUserListItemCount($user_id)
  {
    $this->updateUserListItemCount($user_id);
  }

  public function displayTodoContent($todo_edit_id, $session_id)
  {
    $this->getTodoContent($todo_edit_id, $session_id);
  }

  public function displayUpdatedTodo($todo_id, $todo_content_updated)
  {
    $this->updateTodo($todo_id, $todo_content_updated);
  }

  public function displayUpdatedTodoIsCompleted($is_completed)
  {
    $this->updateTodoIsCompleted($is_completed);
  }

  public function displayTodosNotComplete($session_id)
  {
    $this->getTodosNotCompleted($session_id);
  }

  public function displayTodosIsComplete($session_id)
  {
    $this->getTodosIsCompleted($session_id);
  }

  public function changeTodoToNotCompleted($user_id)
  {
    $this->undoTodoIsCompleted($user_id);
  }

  public function deleteTodo($todo_id, $user_id) 
  {
    $this->removeTodoItem($todo_id, $user_id);
  }
